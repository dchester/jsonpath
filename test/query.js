...
suite('query', function () {
  ...
  test('all things in store', function () {
    ...
    assert.deepEqual(results, [
      ...
      { path: ['$', 'store', '书籍'], value: data.store.书籍 }
    ]);
  });
  ...
  test('all elements', function () {
    ...
    assert.deepEqual(results, [
      ...
      { path: ['$', 'store', 'bicycle', 'price'], value: 19.95 },
      { path: ['$', 'store', '书籍', 0], value: { '分类': '小说', '作者': '王小波', '书名': '一只特立独行的猪' } },
      { path: ['$', 'store', '书籍', 1], value: { '分类': '史书', '作者': '史记', '书名': '司马迁' } },
      { path: ['$', 'store', '书籍', 0, '分类'], value: '小说' },
      { path: ['$', 'store', '书籍', 0, '作者'], value: '王小波' },
      { path: ['$', 'store', '书籍', 0, '书名'], value: '一只特立独行的猪' },
      { path: ['$', 'store', '书籍', 1, '分类'], value: '史书' },
      { path: ['$', 'store', '书籍', 1, '作者'], value: '史记' },
      { path: ['$', 'store', '书籍', 1, '书名'], value: '司马迁' },
    ]);
  });
  ...
  test('object subscript wildcard', function () {
    ...
    assert.deepEqual(results, [data.store.book, data.store.bicycle, data.store.书籍]);
  });
